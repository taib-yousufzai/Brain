...
export default function ToolLibraryPanel({ tools, onDeleteTool }: ToolLibraryPanelProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 font-sans">
      ...
      <div className="bg-[#131823] border border-[#1e2638] p-5 rounded-lg space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <input
            ...
            className="w-full md:w-96 bg-[#0b0f17] border border-slate-700 rounded-md px-3.5 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 font-sans"
          />
          ...
          {ALL_DOMAINS.map((domain) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-mono transition-colors cursor-pointer whitespace-nowrap ${
                  isActive
      ...
      {/* Structured Data Table List (No 3-column cards, No Bento grid) */}
      {filteredTools.length === 0 ? (
        <div className="bg-[#131823] border border-[#1e2638] p-10 rounded-lg text-center space-y-3">
        ...
        <div className="bg-[#131823] border border-[#1e2638] rounded-lg overflow-hidden">
